#!/usr/bin/env bash
#
# sync-farp-data.sh - Refresh FARP entries in data/locations.toml from the
# RotorHeads live map feed and manage the resulting pull request.
#
...
#
# Usage: sync-farp-data.sh [OPTIONS]
#
...
readonly LOCATIONS_FILE="data/locations.toml"
readonly PR_LABEL="farp-data"

BRANCH="${FARP_BRANCH:-automation/farp-data}"
...
cleanup() {
    ...
    [[ -n "${OUTPUTS_FILE}" && -f "${OUTPUTS_FILE}" ]] && rm -f "${OUTPUTS_FILE}"
    [[ -n "${PR_LOCATIONS_FILE}" && -f "${PR_LOCATIONS_FILE}" ]] && rm -f "${PR_LOCATIONS_FILE}"
...
Regenerate FARP data in ${LOCATIONS_FILE} and open or replace a pull request.
...
# Copy the open PR's version of the locations file into PR_LOCATIONS_FILE, so
# the generator can carry manual fields across from it and compare against it.
fetch_pr_locations() {
    PR_LOCATIONS_FILE="$(mktemp)"

    if ! git fetch --quiet origin "${BRANCH}" 2>/dev/null; then
        log_warning "Could not fetch ${BRANCH}; treating the open pull request as empty"
        rm -f "${PR_LOCATIONS_FILE}"
        PR_LOCATIONS_FILE=""
        return 1
    fi
    if ! git show "FETCH_HEAD:${LOCATIONS_FILE}" >"${PR_LOCATIONS_FILE}" 2>/dev/null; then
        log_warning "${BRANCH} has no ${LOCATIONS_FILE}"
        rm -f "${PR_LOCATIONS_FILE}"
        PR_LOCATIONS_FILE=""
        return 1
    fi
...
# Read a single-line key=value written by update_locations.py.
generator_output() {
    local key="$1"
    sed -n "s/^${key}=//p" "${OUTPUTS_FILE}" | head -n 1
}
...
build_pr_body() {
    ...
    cat <<EOF
Automated update of FARP data in \`${LOCATIONS_FILE}\`.
...
- Built by \`scripts/update_locations.py\` from base commit \`${base_sha}\`.
- Airport entries are never touched by this automation.
...
main() {
    ...
    if [[ ! -f "${LOCATIONS_FILE}" ]]; then
        log_error "Missing ${LOCATIONS_FILE} in ${REPO_ROOT}"
        exit 1
    fi
    if [[ ! -f "${MAP_DATA}" ]]; then
    ...
    if ! git diff --quiet -- "${LOCATIONS_FILE}"; then
        log_error "${LOCATIONS_FILE} has uncommitted changes; refusing to run"
        exit 1
    fi
    ...
    if [[ -n "${existing_pr}" ]]; then
        fetch_pr_locations || true
    fi

    local generator_args=("${SCRIPT_DIR}/update_locations.py"
                          --locations "${LOCATIONS_FILE}"
                          --report "${REPORT_FILE}"
                          --commit-message "${COMMIT_MESSAGE_FILE}"
                          --github-output "${OUTPUTS_FILE}")
    if [[ -n "${PR_LOCATIONS_FILE}" ]]; then
        generator_args+=(--carry-from "${PR_LOCATIONS_FILE}"
                         --compare-farps "${PR_LOCATIONS_FILE}")
    fi
    generator_args+=(--map-data "${MAP_DATA}")
    [[ ${QUIET} -eq 1 ]] && generator_args+=(--quiet)
    [[ ${VERBOSE} -eq 1 ]] && generator_args+=(--verbose)
    ...
    if ! python3 "${generator_args[@]}"; then
        log_error "Data generation failed; leaving ${LOCATIONS_FILE} untouched"
        git checkout -- "${LOCATIONS_FILE}"
        exit 1
    fi
    ...
    if git diff --quiet -- "${LOCATIONS_FILE}"; then
        ...
        if [[ -n "${existing_pr}" ]]; then
            close_pr "${existing_pr}" \
                "Closing automatically: FARP data on the base branch now matches the live map feed."
        fi
        return 0
    fi

    log_info "FARP data changed:"
    [[ ${QUIET} -eq 0 ]] && cat "${REPORT_FILE}"

    if [[ -n "${existing_pr}" ]]; then
        local farps_match
        farps_match="$(generator_output farps_match)"

        if [[ "${farps_match}" == "true" ]] && ! pr_has_conflicts "${existing_pr}"; then
            # The base branch may have moved underneath it (an airport edit, say)
            # but GitHub will three-way merge that cleanly, so leave it alone
            # rather than churning a pull request for an unrelated change.
            log_info "Open pull request #${existing_pr} already carries this FARP data; nothing to do"
            git checkout -- "${LOCATIONS_FILE}"
            return 0
        fi

        local close_reason
        if [[ "${farps_match}" == "true" ]]; then
    ...
    if [[ ${DRY_RUN} -eq 1 ]]; then
        ...
        git --no-pager diff -- "${LOCATIONS_FILE}" || true
