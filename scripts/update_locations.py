#!/usr/bin/env python3
"""
update_locations.py - Rebuild the FARP entries in data/locations.toml from the
RotorHeads live map JSON feed.

Airport entries are passed through untouched. FARP entries are regenerated from
the feed, except for fields the feed does not expose (currently elevation),
which are carried over from the existing file.

Usage: update_locations.py [OPTIONS]
"""
...
# coordinate currently in locations.toml (dE = -100.0 m, dN = 0 m, n=8).
...
# Top-level key order inside [[location]].
LOCATION_KEY_ORDER = (
    "type",
    "name",
    "shortname",
...
# FARPs new to the file; an edit made directly in locations.toml also sticks,
...
# be indistinguishable from missing data. scripts/lint_locations.py rejects it,
# and the kneeboard template's `if` guard renders a blank cell rather than a
# misleading zero.
UNKNOWN_ELEVATION = ""

log = logging.getLogger("update_locations")
...
def format_inline_table(table: dict[str, Any], key_order: tuple[str, ...]) -> str:
    ordered = ordered_keys(table, key_order)
    body = ", ".join(f"{key} = {format_value(table[key])}" for key in ordered)
    return "{ " + body + " }"
...
def emit_key_values(
    ...
    for key in ordered_keys(table, key_order):
        if key in skip or isinstance(table[key], (dict, list)):
            continue
        comment = comments.get(key, "")
        lines.append(f"{key} = {format_value(table[key])}{comment}")


def emit_location(location: dict[str, Any]) -> list[str]:
    lines: list[str] = []
    ...
    comments = {}
    if location.get("type") == "farp" and location.get("elevation_feet") == UNKNOWN_ELEVATION:
        comments["elevation_feet"] = "  # TODO: survey elevation in-game"
    emit_key_values(lines, location, LOCATION_KEY_ORDER, comments=comments)

    coordinates = location.get("coordinates", {})
    if "dms" in coordinates:
        lines.append("")
        lines.append("[location.coordinates.dms]")
        for axis in ("latitude", "longitude"):
            if axis in coordinates["dms"]:
                inline = format_inline_table(
                    coordinates["dms"][axis], ("direction", "degrees", "minutes", "seconds")
                )
                lines.append(f"{axis} = {inline}")
    if "mgrs" in coordinates:
        lines.append("")
        lines.append("[location.coordinates.mgrs]")
        emit_key_values(
            lines,
            coordinates["mgrs"],
            ("zone_number", "zone_band", "grid", "easting", "northing"),
        )

    if location.get("navaids"):
        lines.append("")
        lines.append("[location.navaids]")
        emit_key_values(lines, location["navaids"], NAVAID_KEY_ORDER)

    if location.get("frequencies"):
        lines.append("")
        lines.append("[location.frequencies]")
        emit_key_values(lines, location["frequencies"], FREQUENCY_KEY_ORDER)

    for runway in location.get("runways", []):
        lines.append("")
        lines.append("[[location.runways]]")
        emit_key_values(lines, runway, RUNWAY_KEY_ORDER)
        for end in ("primary", "secondary"):
            if end in runway:
                lines.append(f"{end} = {format_inline_table(runway[end], ('name', 'heading'))}")

    return lines


def emit_locations(locations: list[dict[str, Any]]) -> str:
    """Render the whole file.

    Names are upper-cased here rather than at the point they are read, so the
    rule covers hand-written airports as well as generated FARPs, and so the
    linter's canonical-format check catches any that slip through. Sorting
    happens after, or the order would depend on the case it arrived in.
    """
    normalised = sorted(
        ({**location, "name": str(location["name"]).upper()} for location in locations),
        key=lambda location: location["name"],
    )
    blocks = [emit_location(location) for location in normalised]
...
def merge_locations(
    ...
    Airports always come from `existing` (the base branch). FARP identity and
    ...
    by_upper_name = {
        location["name"].upper(): location
        for location in carry_source
        if location.get("type") == "farp"
    }

    merged: list[dict[str, Any]] = [
        location for location in existing if location.get("type") != "farp"
    ]
...
def describe_changes(
    ...
    def farps(locations: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
        return {
            location["name"].upper(): location
    ...
    rebuilt = rebuilt or {}
    before, after = farps(existing), farps(updated)
...
def farp_entries(locations: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [location for location in locations if location.get("type") == "farp"]
...
def parse_arguments(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="update_locations.py",
        description="Rebuild FARP entries in locations.toml from the RotorHeads map feed.",
    )
    parser.add_argument(
        "--map-data",
        type=Path,
    ...
    parser.add_argument("--locations", type=Path, default=Path(DEFAULT_LOCATIONS_PATH))
    parser.add_argument("-o", "--output", type=Path, help="write here instead of in place")
    parser.add_argument(
        "--carry-from",
        type=Path,
        help="take manual FARP fields (elevation, shortname) from this file instead "
        "of --locations; point it at an open pull request's copy of the file",
    )
    parser.add_argument(
        "--compare-farps",
        type=Path,
        help="also report whether the generated FARP entries match this file's, "
        "as the `farps_match` output",
    )
...
def main(argv: list[str]) -> int:
    ...
    if not args.locations.is_file():
        log.error("Locations file not found: %s", args.locations)
        return 2

    original_text = args.locations.read_text(encoding="utf-8")
    existing = tomllib.loads(original_text).get("location", [])
    ...
    updated_text = emit_locations(updated)
    report_lines, counts, changed_names = describe_changes(existing, updated, rebuilt)

    # Airports moving on the base branch must not, on its own, invalidate an open
    # pull request, so the comparison looks at FARP entries rather than bytes.
    farps_match = "unknown"
    if args.compare_farps and args.compare_farps.is_file():
        other = tomllib.loads(args.compare_farps.read_text(encoding="utf-8")).get("location", [])
        farps_match = "true" if farp_entries(other) == farp_entries(updated) else "false"
    ...
    if changed and not report_lines:
        ...
        summary = "Regenerated `locations.toml`; no FARP data changed."
    ...
    if not args.dry_run:
        destination = args.output or args.locations
